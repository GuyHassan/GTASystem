...
const AddingStudentToClass = ({ match: { params } }) => {
        ...
        return Object.values(studentExists).map(student => {
            return (
                <div className="item" key={student.id}>
                    <ul>
                        <li onClick={() => listNewStudent(student.name)}
                            style={{ color: newStudent.includes(student.name) ? "red" : "blue", fontSize: '20px' }}>
    ...
    const addNewStudent = () => {
        ...
        const details = { lecturerName: user, professionName: params.profession, students: studentExists }
